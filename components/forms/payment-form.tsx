...
export function PaymentForm({ action, folios }: PaymentFormProps) {
  ...
  return (
    <form action={action} className="mt-4 grid gap-3 md:grid-cols-5" onSubmit={() => setSubmitAttempted(true)} noValidate>
      ...
      <input
        name="notes"
        placeholder="Notas"
        className="rounded-lg border border-border-soft px-3 py-2 text-sm md:col-span-5"
      />
      <label className="md:col-span-5 flex items-center gap-2 rounded-lg border border-border-soft px-3 py-2 text-sm text-text-muted">
