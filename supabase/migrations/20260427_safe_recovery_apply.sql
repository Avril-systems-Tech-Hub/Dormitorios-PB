-- Safe recovery/apply script for environments with partial migration state.
...
begin
  ...
  if not exists (select 1 from pg_type where typname = 'guest_sex') then
  ...
  if not exists (select 1 from pg_type where typname = 'cash_movement_category') then
  ...
  if not exists (select 1 from pg_type where typname = 'anomaly_flag') then
...
begin
  ...
  if status_udt = 'whatsapp_status' then
    update public.whatsapp_messages
    set status = case
      when lower(status::text) in ('queued', 'sent', 'failed') then lower(status::text)::public.whatsapp_status
      else 'queued'::public.whatsapp_status
    end
    where status is not null;
  else
    update public.whatsapp_messages
    set status = case
      when lower(status::text) in ('queued', 'sent', 'failed') then lower(status::text)
      else 'queued'
    end
    where status is not null;
  end if;
...
drop policy if exists "ops_select_all" on public.guests;
create policy "ops_select_all"
on public.guests for select
using (auth.uid() is not null);

drop policy if exists "ops_modify_guests" on public.guests;
...
drop policy if exists "ops_beds_access" on public.beds;
create policy "ops_beds_access"
on public.beds for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "ops_folios_access" on public.folios;
create policy "ops_folios_access"
on public.folios for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "ops_reservations_access" on public.reservations;
create policy "ops_reservations_access"
on public.reservations for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "ops_reservation_guests_access" on public.reservation_guests;
...
drop policy if exists "ops_shifts_access" on public.shifts;
create policy "ops_shifts_access"
on public.shifts for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "ops_cash_cuts_access" on public.cash_cuts;
create policy "ops_cash_cuts_access"
on public.cash_cuts for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "admin_audit_access" on public.audit_logs;
create policy "admin_audit_access"
on public.audit_logs for all
using (public.current_role() = 'admin')
with check (public.current_role() = 'admin');

drop policy if exists "ops_promotion_claims_access" on public.promotion_claims;
create policy "ops_promotion_claims_access"
on public.promotion_claims for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "ops_whatsapp_messages_access" on public.whatsapp_messages;
create policy "ops_whatsapp_messages_access"
on public.whatsapp_messages for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "ops_cash_movements_access" on public.cash_movements;
create policy "ops_cash_movements_access"
on public.cash_movements for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "ops_import_batches_access" on public.import_batches;
create policy "ops_import_batches_access"
on public.import_batches for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "ops_imported_records_access" on public.imported_records;
create policy "ops_imported_records_access"
on public.imported_records for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "ops_imported_record_anomalies_access" on public.imported_record_anomalies;
create policy "ops_imported_record_anomalies_access"
on public.imported_record_anomalies for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "ops_extra_services_access" on public.extra_services;
create policy "ops_extra_services_access"
on public.extra_services for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "ops_folio_extra_services_access" on public.folio_extra_services;
create policy "ops_folio_extra_services_access"
on public.folio_extra_services for all
using (public.current_role() in ('admin', 'reception'))
with check (public.current_role() in ('admin', 'reception'));

drop policy if exists "ops_imported_record_extra_services_access" on public.imported_record_extra_services;
