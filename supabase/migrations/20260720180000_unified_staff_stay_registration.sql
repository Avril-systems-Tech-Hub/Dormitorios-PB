-- One transactional staff entry point for new, in-progress and finished stays.
-- Imported payments keep their real effective date and never enter today's shift.

alter table public.reservations
  add column if not exists registration_mode text;

alter table public.reservations
  drop constraint if exists reservations_registration_mode_check,
  add constraint reservations_registration_mode_check
    check (registration_mode is null or registration_mode in ('new', 'current', 'finished'));

create table if not exists public.stay_registration_submissions (
  id uuid primary key,
  actor_user_id uuid references public.profiles(id) on delete set null,
  mode text not null check (mode in ('new', 'current', 'finished')),
...
alter table public.stay_registration_submissions enable row level security;

create index if not exists idx_stay_registration_submissions_actor
  on public.stay_registration_submissions (actor_user_id, created_at desc);

create or replace function public.preserve_registered_stay_lifecycle()
returns trigger
language plpgsql
as $$
begin
  if old.registration_mode in ('new', 'current')
     and old.status = 'active'
     and new.status = 'confirmed'
     and new.checked_out_at is null then
    new.status := 'active';
  end if;
  return new;
end;
...
drop trigger if exists reservations_preserve_registered_lifecycle on public.reservations;
create trigger reservations_preserve_registered_lifecycle
before update on public.reservations
for each row execute function public.preserve_registered_stay_lifecycle();
...
begin
  ...
  begin
    ...
    v_total := round(coalesce((p_payload->>'total_amount')::numeric, 0), 2);
  ...
  if v_total < 0 or v_payment < 0 or v_payment > v_total then
    raise exception 'El total o el pago no son válidos.' using errcode = '22023';
  end if;
  ...
  loop
    ...
    v_locker_days := case
      when p_mode = 'finished' then 0
      else greatest(0, least(v_nights, coalesce((v_guest->>'locker_days')::integer, 0)))
    end;
    ...
    insert into public.reservation_guests (
      ...
      v_final_rate,
      v_locker_number,
      case when p_mode = 'new' and v_locker_days > 0 then 30 else 0 end,
      v_locker_days,
      case when p_mode = 'new' then v_locker_days * 30 else 0 end,
      case when p_mode = 'finished' then 'historical' else 'captured' end
    );
  end loop;

  -- Preserve an imported in-progress folio total exactly for later payments.
  if p_mode = 'current' and v_total > 0 then
    insert into public.folio_extra_services (
      folio_id, extra_service_id, service_name, amount, notes, created_by
    ) values (
      v_folio_id, null, 'Total de estancia vigente importada', v_total,
      'Importe original conservado por registro unificado.', v_actor.id
    );
  end if;
...
revoke all on function public.register_staff_stay(uuid,text,jsonb) from public;
grant execute on function public.register_staff_stay(uuid,text,jsonb) to authenticated;

comment on function public.register_staff_stay(uuid,text,jsonb) is
  'Atomic and idempotent registration for new, current and finished staff stays.';

-- Retire the separate historical write path; the unified RPC now owns all staff creation.
