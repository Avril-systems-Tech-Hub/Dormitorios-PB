-- Align stay period to business policy: 11:00 → 11:00 America/Mexico_City.
-- Bed frees at check-out 11:00; formal checkout remains a reception confirmation.
...
comment on function public.register_staff_stay(uuid,text,jsonb) is
  'Atomic and idempotent registration for new, current and finished staff stays with auto-calculated totals. Missing folio codes are assigned as FPB- (new) or IMP- (imported). Duplicate phones create a new guest unless match_decision is reuse.';
