-- Staff stays charge the admin catalog instead of hardcoded 120/30.
...
declare
  ...
  v_locker_daily numeric(10,2) := 30;
...
begin
  ...
  select amount into v_nightly_rate
  from public.service_prices
  where key = 'bed_night';
  if v_nightly_rate is null or v_nightly_rate < 0 then
    v_nightly_rate := 120;
  end if;
  select amount into v_locker_daily
  from public.service_prices
  where key = 'guest_locker_day';
  if v_locker_daily is null or v_locker_daily < 0 then
    v_locker_daily := 30;
  end if;
  ...
  select coalesce(sum(
    greatest(0, least(v_nights, coalesce((guest->>'locker_days')::integer, 0))) * v_locker_daily
  ), 0)
  into v_locker_total
  from jsonb_array_elements(p_payload->'guests') guest;
  ...
  insert into public.reservations (
    ...
    v_check_in,
    v_check_out,
    (v_check_in + time '11:00') at time zone 'America/Mexico_City',
    (v_check_out + time '11:00') at time zone 'America/Mexico_City',
    case when p_mode = 'finished'
      then (v_check_out + time '11:00') at time zone 'America/Mexico_City'
      else null end,
  ...
  loop
    ...
    insert into public.reservation_guests (
      ...
      v_final_rate,
      v_locker_number,
      case when v_locker_days > 0 then v_locker_daily else 0 end,
      v_locker_days,
      v_locker_days * v_locker_daily,
...
comment on function public.register_staff_stay(uuid,text,jsonb) is
  'Atomic and idempotent registration for new, current and finished staff stays. Bed and guest-locker rates come from service_prices.';
