...
/**
 * Envía un documento PDF por WhatsApp usando un template como primer mensaje.
 *
 * Estrategia para llegar a números que nunca han escrito al negocio:
 * 1. Envía un template message (business-initiated) para abrir la ventana de 24h.
 * 2. Inmediatamente después, envía el documento PDF como mensaje libre
 *    (ahora permitido porque la ventana está abierta).
 *
 * Si el template no está configurado, hace fallback a envío directo del documento.
 */
export async function sendWhatsAppDocumentWithTemplate(
  toPhone: string,
  pdfUrl: string,
  filename: string,
  caption: string,
  ...
  const resolvedTemplateName =
    templateName || process.env.YCLOUD_TEMPLATE_NAME || "";
  const resolvedTemplateLanguage =
    templateLanguage || process.env.YCLOUD_TEMPLATE_LANGUAGE || "es";

  // Si hay template configurado, enviarlo primero para abrir la ventana de 24h
  if (resolvedTemplateName) {
    console.log(
      `[YCloud] Enviando template "${resolvedTemplateName}" para abrir ventana de 24h...`,
    );
    ...
    if (!templateResult.success) {
      console.warn(
        "[YCloud] Template falló, intentando envío directo del documento:",
        templateResult.error,
      );
      // Si el template falla, intentar enviar el documento directamente
      // (puede funcionar si el usuario ya tenía una ventana abierta)
    } else {
      // Esperar 12 segundos para que WhatsApp procese el template, lo entregue al teléfono
      // y abra la ventana de 24h antes de enviar el documento PDF
      console.log("[YCloud] Esperando 12s para que WhatsApp entregue el template y abra la ventana de 24h...");
      await new Promise((resolve) => setTimeout(resolve, 12000));
    }
  } else {
    console.warn(
      "[YCloud] No hay template configurado (YCLOUD_TEMPLATE_NAME). " +
        "El envío podría fallar si el usuario no ha enviado un mensaje previo.",
    );
  }

  // Enviar el documento PDF con reintentos
  // WhatsApp puede tardar en registrar la ventana de 24h después del template,
  // así que reintentamos hasta 3 veces con espera progresiva
  const maxRetries = 3;
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    if (docResult.success) {
      console.log(`[YCloud] Documento enviado exitosamente (intento ${attempt + 1})`);
      return docResult;
    }
    ...
    if (attempt < maxRetries) {
      ...
      console.warn(
        `[YCloud] Documento falló (intento ${attempt + 1}/${maxRetries + 1}), reintentando en ${delay / 1000}s... Error: ${docResult.error}`,
      );
      await new Promise((resolve) => setTimeout(resolve, delay));
    } else {
      console.error(`[YCloud] Documento falló después de ${maxRetries + 1} intentos.`);
      return docResult;
    }
  }
